...
var textpattern = (function () {
  ...
  var $_egs2jgrsjn69h5ax = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_byapo8s0jn69h5bk.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_byapo8s0jn69h5bk.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_byapo8s0jn69h5bk.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_ambfnprxjn69h5b9.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_ambfnprxjn69h5b9.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_ambfnprxjn69h5b9.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_5y4vhbrwjn69h5b4.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_5y4vhbrwjn69h5b4.checkKeyCode(keyCodes, e)) {
        $_5y4vhbrwjn69h5b4.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_5y4vhbrwjn69h5b4.checkCharCode(charCodes, e)) {
  ...
  var $_6q3ttcrtjn69h5az = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_egs2jgrsjn69h5ax.getPatterns(editor.settings));
    $_6q3ttcrtjn69h5az.setup(editor, patternsState);
    return $_as6572rrjn69h5aw.get(patternsState);
  });
  function Plugin () {
  }
