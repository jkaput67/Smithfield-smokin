...
var searchreplace = (function () {
  ...
  var $_2g3w9sk0jn69h2fb = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_2g3w9sk0jn69h2fb.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_gav3ucjyjn69h2f5.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_gav3ucjyjn69h2f5.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_gav3ucjyjn69h2f5.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_gav3ucjyjn69h2f5.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_gav3ucjyjn69h2f5.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_gav3ucjyjn69h2f5.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_gav3ucjyjn69h2f5.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_gav3ucjyjn69h2f5.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_gav3ucjyjn69h2f5.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_gav3ucjyjn69h2f5.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_gav3ucjyjn69h2f5.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_gav3ucjyjn69h2f5.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_gav3ucjyjn69h2f5.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_gav3ucjyjn69h2f5.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_gav3ucjyjn69h2f5.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_eqg6esk2jn69h2fj = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_eqg6esk2jn69h2fj.open(editor, currentIndexState);
    });
  };
  var $_nzxeok1jn69h2fh = { register: register };
  ...
  var $_6vld32k3jn69h2fm = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_nzxeok1jn69h2fh.register(editor, currentIndexState);
    $_6vld32k3jn69h2fm.register(editor, currentIndexState);
    return $_g8eb05jxjn69h2f2.get(editor, currentIndexState);
  });
  function Plugin () {
  }
